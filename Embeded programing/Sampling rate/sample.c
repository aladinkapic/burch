...
float counter = 0, sekunde = 0, counter2 = 0, finalvalue = 0, displayvalue = 0;
...
void __interrupt () my_isr_routine (void){  
    if(TMR0IF==1){
        ...
        if(counter == 74){
            ...
            if (counter2 < MAX_ADC_VALUES){
                finalvalue = finalvalue + val;
                counter2++;
            }else{
                displayvalue = finalvalue/5;
               
            finalvalue = 0;
            counter2 = 0;
            }
            char buf[30];
            sprintf (buf, "A = %f",  displayvalue);
